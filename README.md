...
## Workflows with Original Paths

These are the concrete commands as run on the lab machine. Adapt paths if your clone location differs.

### Regenerating the main input file with different constants

Navigate to `saltyBiomass/software_module/pflotran/generator/`. Change the constants you want using the dictionaries at the top of `pflotran_generator.py`. Default output is `.tec` files. Follow the comments in the generator if you want HDF5 output (recommended for multi-salinity runs).

### Generating files with different salt concentrations

Navigate to `saltyBiomass/software_module/pflotran/batch/`. Copy in the `9_addnitrogen.in` generated earlier (or use the one already there if you haven't changed constants). Run:

```bash
python create_modified_files.py
```

By default it creates 20 files from 1× to 20× seawater. The seawater ion concentrations were calculated from: https://docs.google.com/spreadsheets/d/1iNVlg_OOcvQkkKXAuV_2iWS9l-c619CPPVjG7pkcaQE/edit?pli=1&gid=0#gid=0

### Running multiple files

```bash
cd saltyBiomass/software_module/pflotran/batch/
...
```bash
cd saltyBiomass/software_module/pflotran/visualization/
...
Run the notebook at `saltyBiomass/software_module/pflotran/compare/comparing_aw.ipynb`. Update `working_directory` to point to your `.tec` output directory:

```python
working_directory = "saltyBiomass/software_module/pflotran/batch/modified_pflotran_files"
```
