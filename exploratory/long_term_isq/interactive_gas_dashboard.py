...
from bokeh.models import (Select, Slider, Button, Div, HoverTool, 
...
def load_and_process_data(filename):
    ...
    try:
        ...
        print(f"Sample IDs: {df['Sample ID'].unique()[:10] if 'Sample ID' in df.columns else 'No Sample ID column'}")
        
        # Convert numeric columns and handle non-numeric values
        df['Calc Concentration (ppm)'] = pd.to_numeric(df['Calc Concentration (ppm)'], errors='coerce')
        ...
        mean_data = df.groupby(['experiment_condition', 'days_since_start', 'Gas']).agg({
            'Calc Concentration (ppm)': ['mean', 'std'],
            'Water Activity': 'mean'
        }).reset_index()
        
        # Flatten column names
        mean_data.columns = ['experiment_condition', 'days_since_start', 'Gas', 
                           'mean_conc_ppm', 'std_conc_ppm', 'mean_water_activity']
        
        # Create display labels using water activity when available
        mean_data['display_label'] = mean_data.apply(
            lambda row: create_experiment_label(row['experiment_condition'], row['mean_water_activity']), 
            axis=1
        )
        
        print(f"Processed data: {len(mean_data)} rows")
        print(f"Experiment conditions: {sorted(mean_data['experiment_condition'].unique())}")
        ...
        mean_data['std_conc_molL'] = mean_data.apply(
            lambda row: ppm_to_mol_per_L(row['std_conc_ppm'], row['Gas']) 
            if not pd.isna(row['std_conc_ppm']) else np.nan, axis=1
        )
...
def create_plots_by_treatment(data, projection_days):
    ...
    for condition in selected_conditions:
        ...
        display_label = condition_data['display_label'].iloc[0] if len(condition_data) > 0 else condition
...
def create_plots_by_gas(data, projection_days):
    ...
    for gas in gases:
        if gas in gas_colors:
            ...
            for i, condition in enumerate(unique_conditions[:10]):  # Limit to 10 for visibility
                condition_data = data[(data['experiment_condition'] == condition) & 
                                    (data['Gas'] == gas)]
...
def update_plots():
    ...
    water_activity_labels = current_data['display_label'].str.contains('water_activity').sum()
...
dataset_select = Select(
    ...
    value="methanogen",
    options=[("methanogen", "Methanogen Data"), 
             ("spirulina", "Spirulina Data"), 
             ("mix", "Mix Data")]
)

projection_slider = Slider(
    title="Projection Days:",
    start=30,
    end=3650,
    value=60,
    step=10
...
plot_org_radio = RadioButtonGroup(
    labels=["Organize by Treatment", "Organize by Gas"],
    active=0
)
