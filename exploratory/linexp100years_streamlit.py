...
def exponential_release_rates(pct_total: float, years: float):
    ...
    λ = -math.log(1 - pct_total / constants.percent_conversion) / (
        years * constants.months_p_year
    )
    # now project out:
    pct_year = constants.percent_conversion * (
        1 - math.exp(-λ * constants.months_p_year)
    )
    pct_month = constants.percent_conversion * (1 - math.exp(-λ * 1))
    pct_week = constants.percent_conversion * (
        1 - math.exp(-λ * constants.weeks_p_year)
    )
...
def concentration_metrics(
    volume_mL: float, sample_mL: float = constants.sample_mL_default
):
...
num_years = st.number_input(
    "Years to release:", min_value=1, max_value=10000, value=100, step=1
)
pct_total_y = st.number_input(
    "% released in your set years:",
    min_value=0.0,
    max_value=100.0,
    value=0.01,
    step=0.01,
...
start_carbon = st.number_input(
    "Starting mg carbon:", min_value=0.0, value=875.0, step=0.1
)
