...
def find_target_point(df):
    ...
    target_point = surface_points[
        (surface_points['X [m]'] == min_x) & 
        (surface_points['Y [m]'] == min_y)
    ]
...
def extract_point_time_series(flux_df, target_x, target_y, target_z):
    ...
    if len(point_data) > 0:
        print(f"Time range: {point_data['Time Index'].min()} to {point_data['Time Index'].max()}")
        print(f"CO2 flux range: {point_data['CO2_flux_magnitude'].min():.3e} to {point_data['CO2_flux_magnitude'].max():.3e}")
        print(f"CH4 flux range: {point_data['CH4_flux_magnitude'].min():.3e} to {point_data['CH4_flux_magnitude'].max():.3e}")
...
def create_flux_time_series_plot(point_data, target_x, target_y, target_z):
    ...
    if point_data['CO2_flux_magnitude'].max() < 1e-3 or point_data['CH4_flux_magnitude'].max() < 1e-3:
    ...
    p2.line('Time Index', 'CO2_flux_x', source=source, line_width=2, color='red', alpha=0.7, legend_label="CO2 Flux X")
    p2.line('Time Index', 'CO2_flux_y', source=source, line_width=2, color='orange', alpha=0.7, legend_label="CO2 Flux Y")
    p2.line('Time Index', 'CO2_flux_z', source=source, line_width=2, color='purple', alpha=0.7, legend_label="CO2 Flux Z")
    
    p2.line('Time Index', 'CH4_flux_x', source=source, line_width=2, color='darkgreen', alpha=0.7, legend_label="CH4 Flux X", line_dash='dashed')
    p2.line('Time Index', 'CH4_flux_y', source=source, line_width=2, color='lightgreen', alpha=0.7, legend_label="CH4 Flux Y", line_dash='dashed')
    p2.line('Time Index', 'CH4_flux_z', source=source, line_width=2, color='olive', alpha=0.7, legend_label="CH4 Flux Z", line_dash='dashed')
    
    p2.legend.location = "top_left"
    p2.legend.click_policy = "hide"
    p2.legend.label_text_font_size = "10pt"
    
    if point_data['CO2_flux_x'].abs().max() < 1e-3 or point_data['CH4_flux_x'].abs().max() < 1e-3:
