...
def create_flux_visualization(flux_df):
    ...
    for i, time_idx in enumerate(
        time_indices[:5]
    ):  # Limit to first 5 time steps for testing
        ...
        print(
            f"Time step {time_idx}: Max CO2 flux = {max_co2_flux:.3e}, Max CH4 flux = {max_ch4_flux:.3e}"
        )
        ...
        if co2_flux_max > co2_flux_min and co2_flux_max > 1e-15:
            color_mapper_co2 = linear_cmap(
                "CO2_flux_magnitude", Viridis256, co2_flux_min, co2_flux_max
            )
        ...
        if ch4_flux_max > ch4_flux_min and ch4_flux_max > 1e-25:
            color_mapper_ch4 = linear_cmap(
                "CH4_flux_magnitude", Viridis256, ch4_flux_min, ch4_flux_max
            )
...
def main():
    ...
    if df["Free CH4(aq) [M]"].max() <= 1e-14:
        print(
            "  Note: CH4 concentrations are at numerical floor - little/no methane production yet"
        )
